...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        prog = 'python urgent_ukraine_fetch_chirps.py',
        description = (
            'Urgent script to generate CHIRPS csv for a list of geometries and save them.'
        ),
    ...
    parser.add_argument('export_folderpath', action='store', help='Filepath where the output csv is to be stored.')
    parser.add_argument('filename_col', action='store', help='Column in the roi_filepath file which could be used as filename for the individual csvs.')
    ...
    filename_col = str(args.filename_col)
    ...
    print(f"export_folderpath: {export_folderpath}")
    ...
    for index, row in shapes_gdf.iterrows():
        ...
        print(f'{filename} [{i} / {shapes_gdf.shape[0]}]')
        export_filepath = os.path.join(export_folderpath, f'{filename}.csv')
        _shapes_gdf = gpd.GeoDataFrame(data = {'geometry': [row['geometry']]}, crs=shapes_gdf.crs)
